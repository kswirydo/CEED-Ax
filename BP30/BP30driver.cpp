...
int main(int argc, char **argv){
  ...
  occa::device device;
  occa::kernel BP3kernel[13];
  occa::kernel BP3kernel3D[9];
  ...
  for (int i =1; i<14; i++)
  {
    ...
    BP3kernel[i-1] = device.buildKernelFromSource("BP3kernels2D.okl", buf, kernelInfo);
  ...
  if (p_gjNq<11)
  {
    for (int i =1; i<10; i++)
    {
      ...
      BP3kernel3D[i-1] = device.buildKernelFromSource("BP3kernels3D.okl", buf, kernelInfo);
      //  printf("%s\n", buf); // outputs so you can see it
    }
    printf("compiling 3D control kernel  ...\n");
    correctRes  = device.buildKernelFromSource("BP3kernels3D.okl", "ellipticAxHex3D_e9", kernelInfo);
  ...
  //  randCalloc(device, p_gjNq*p_Nq, &I, o_I);
  // remember: for correct results, we must have I_{nm} = I_{p_gjNq-n+1, p_Nq-m+1}
  ...
  for (int i =1;i<14; i++){
    ...
    for(it=0;it<Niter;++it){
      BP3kernel[i-1](E,o_geo, o_D,o_I, lambda, o_u, o_Autemp,o_Au );
...
if (i<=9){
    ...
    printf("OCCA estimated bandwidth = %17.15f GB/s\n", sizeof(datafloat)*Niter*E*(7.*p_gjNq*p_gjNq*p_gjNq+2.0*p_Nq*p_Nq*p_Nq)/(elapsed*1024.*1024.*1024));
  ...
  if (p_gjNq<11){
    for (int i =1;i<10; i++){
      ...
      for(it=0;it<Niter;++it){
        ...
        occa::streamTag startTag = device.tagStream();
        if ((i >1)&&(i<7))     
          BP3kernel3D[i-1](E,o_geo, o_D,o_I, lambda, o_u, o_Autemp,o_Autemp2, o_Au );
        else            
          BP3kernel3D[i-1](E,o_geo, o_D,o_I, lambda, o_u, o_Autemp, o_Au);
        occa::streamTag stopTag = device.tagStream();
      ...
      //Niter* (12*p_gjNq*p_gjNq*p_gjNq*p_gjNq + 15*p_gjNq*p_gjNq*p_gjNq);i
      //
    ...
    if (p_gjNq<11){  
      ...
      printf("RESULT SHOULD BE: normAu = %17.15lf\n", normAu); 
    }
    printf("\n");
  }
